...
def summarize():
    if 'username' in session:
        if request.method == 'POST':
            ...
            # Tokenize input text
            inputs = tokenizer(pubmed_text, return_tensors="pt")

            # Generate summary
            summary_ids = model.generate(inputs['input_ids'], max_length=150, min_length=0, length_penalty=2.0, num_beams=4, early_stopping=True)
